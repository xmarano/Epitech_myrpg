...
int import_weapons_stats(Global_t *m)
{
    ...
    for (int i = 0; i < 15; i++) {
        if (fscanf(file, "%s %d %d %d %d %d", m->weapons[i].name, &m->weapons[i].attack, &m->weapons[i].accuracy, &m->weapons[i].rng, &m->weapons[i].crit, &m->weapons[i].cost) != 6) {
            fprintf(stderr, "Erreur de lecture pour l'arme %d\n", i);
            fclose(file);
            return 84;
        }
    }
