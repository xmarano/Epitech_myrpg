...
int draw_rain(Global_t *m)
{
    ...
    for (int i = 0; i < NUM_PARTICLES; ++i) {
        ...
        sfVertex vertex1 = {.position = m->hub.particles[i].position,
        .color = sfColor_fromRGB(0, 192, 255)};
        sfVertex vertex2 = {.position = {m->hub.particles[i].position.x + 2,
        m->hub.particles[i].position.y + 10},
        .color = sfColor_fromRGB(0, 192, 255)};
