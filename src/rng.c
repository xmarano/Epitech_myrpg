...
int is_hit(stchar_t *attack, stchar_t *defense, Weapons_t *weapon_atk, Weapons_t *weapon_def)
{
    int nb1 = rand() % 100;
    int nb2 = rand() % 100;
    int accuracy = weapon_atk->accuracy + attack->skl * 2 + attack->lck * 2 + is_weapon_advantage(weapon_atk, weapon_def) * 10;
    int avoid = defense->spd * 2 + defense->lck;
